...
def format_discord_message(news_item, google_news, keyword, country_name, flag, formatted_date):
    ...
    try:
        ...
        with sqlite3.connect(DB_PATH) as conn:
            if INITIALIZE_KEYWORD:
                ...
                new_items = [item for item in news_items if not is_guid_posted(item.find('guid').text, conn)]
        ...
        for item in new_items:
            try:
                ...
                discord_message = format_discord_message(news_item, google_news, keyword, country_name, flag, formatted_date)
